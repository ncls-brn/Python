...
# Fonction pour supprimer un vaisseau si son équipage est vide
def remove_spaceship(fleet):
    if not fleet.spaceships:
        print("Aucun vaisseau dans la flotte.")
        return

    print("Sélectionnez un vaisseau à supprimer:")
    for i, spaceship in enumerate(fleet.spaceships, 1):
        print(f"{i}. {spaceship.name} - Équipage: {len(spaceship.crew)} membres")
    ...
    if len(spaceship.crew) == 0:
        fleet.spaceships.remove(spaceship)
        print(f"Le vaisseau {spaceship.name} a été supprimé de la flotte.")
    else:
...
def main():
    ...
    while True:
        ...
        print("10. Supprimez un vaisseau")
